          ...
          <div className="flex flex-col h-[500px]">
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-3">
              {messages.length === 0 ? (
                <div className="flex items-center justify-center h-full">
                  <p className="text-gray-400 text-[14px]">No messages yet. Send a message to start the conversation.</p>
                </div>
              ) : (
                messages.map((m) => {
                  ...
                  return (
                    <div key={m.id} className={`flex ${isAdmin ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${
                        isAdmin ? 'bg-gray-100 text-gray-800' : 'bg-[#0c1940] text-white'
                      }`}>
                        <p className={`text-[11px] font-[700] mb-1 ${isAdmin ? 'text-gray-500' : 'text-blue-200'}`}>
                          {isAdmin ? 'RepuKeel Team' : 'You'}
                        </p>
                        <p className="text-[14px] leading-relaxed">{m.message}</p>
                        <p className={`text-[11px] mt-1 ${isAdmin ? 'text-gray-400' : 'text-blue-300'}`}>
            ...
            <div className="border-t border-gray-100 p-4 flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-[#f8fafc] border border-gray-200 rounded-xl text-[14px] outline-none focus:border-[#d4af37] focus:ring-1 focus:ring-[#d4af37] transition-all"
              />
              <button
                onClick={handleSendMessage}
                disabled={sendingMsg || !newMessage.trim()}
                className="bg-[#d4af37] hover:bg-[#c19b2e] text-white font-[700] px-5 py-2.5 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {sendingMsg ? '...' : 'Send'}
              </button>
            </div>
